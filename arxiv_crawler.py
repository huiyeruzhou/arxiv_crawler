...
class ArxivScraper:
    def __init__(self, date_from, date_until, 
                 category_blacklist=[], 
                 category_whitelist=["cs.CV", "cs.AI", "cs.LG", "cs.CL"],
                 oprional_keywords=["LLM", "language model", "multimodal", "finetuning", "GPT"],
                 ):
        """
    ...
    def get_url(self, start):
        ...
        kwargs = ''.join(f'&terms-{i}-operator=OR&terms-{i}-term={kw}&terms-{i}-field=all' for i, kw in enumerate(self.oprional_keywords))
        return f'https://arxiv.org/search/advanced?advanced={kwargs}' + f'&classification-computer_science=y&classification-physics_archives=all&classification-include_cross_list=include&date-year=&date-filter_by=date_range&date-from_date={self.date_from}&date-to_date={self.date_until}&date-date_type={self.filter_date_by}&abstracts=show&size={self.step}&order={self.order}&start={start}'
    ...
    async def fetch_all(self):
        ...
        with Progress(
            ...
            *Progress.get_default_columns(),
            ...
            task = p.add_task(description=f"[bold green]Fetching {self.total} results", total=self.total)
        ...
        self.console.log(f"[bold green]Fetching completed. Papers: {len(self.papers)} ({len(self.filtered_papers)} filtered)")
    ...
    def parse_search_html(self, content):
        """
        解析搜索结果页面, 并将结果保存到self.papers和self.filtered_papers中
        初次调用时, 会解析self.total
        ...
        for result in results:
            ...
            categories = [category.get_text(strip=True) for category in category_tag]
            filtered = False
            ...
            abstract = summary_tag.get_text(strip=True)[:-len("△ Less")] if summary_tag else 'No summary'
        ...
        self.filtered_papers.extend(filtered_papers)

    @staticmethod
    def print_paper(paper, file):
        """
        打印一篇论文的信息
    ...
    @staticmethod
    def print_filtered_paper(paper, file):
        """
        打印一篇过滤的论文的信息
    ...
    async def translate(self):
        ...
        with Progress(
            ...
            *Progress.get_default_columns(),
            ...
            task = p.add_task(description=f"[bold green]Translating {len(self.papers)} papers", total=len(self.papers))
            async def wrapper(paper):
                paper['标题'] = await async_translate(paper['title'])
                paper['摘要'] = await async_translate(paper['abstract'])
                p.update(task, advance=1)
                
            translates = [wrapper(paper) for paper in self.papers]
    ...
    def output(self, filename_format='%Y-%m-%d'):
        ...
        for paper in self.papers:
            ...
            if date not in files:
                files[date] = open(self.output_dir / f"{self.convert_date(date, filename_format)}.md", 'w', encoding='utf-8')
        ...
        self.console.log(f"[bold green]Output completed. Files: {len(files)}")
